...
namespace Services.Services
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public  async Task SoftDeleteAsync(int? id) => await _employeeRepo.SoftDeleteAsync(id);
        public async Task<IEnumerable<EmployeeDto>> SearchAsync(string searchText)
        {
            if (string.IsNullOrWhiteSpace(searchText))
            {
                return _mapper.Map<IEnumerable<EmployeeDto>>(await _employeeRepo.GetAllAsync());
            }
            return _mapper.Map<IEnumerable<EmployeeDto>>(await _employeeRepo.GetAllAsync(e => e.FullName.Contains(searchText)));
        }
    }
}
